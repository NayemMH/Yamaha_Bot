...
- **`ChatbotView.tsx`** — the main AI chat surface. Maintains its own `messages: ChatMessage[]` state (not persisted server-side — refreshing the page loses chat history) and posts to `/api/chat`. Renders `purchaseLeadRef`/`bikeCard`/`appointmentRef` inline as rich cards when a message carries one (see `ChatMessage` in `types.ts`). Owns an in-chat conversational lead-collection flow (`leadFlow` state machine: bot asks name → phone → location as chat messages, with a location-autocorrect confirm step) shared by both bike purchases (posts to `/api/purchase-lead`) and AI-diagnosed product/service recommendations (posts to `/api/service-consultation`) — no popup modal.
